#!/usr/bin/env python3
"""
Deploy Planner Agent to Cloud Run with A2A support.
This script deploys the Planner Agent as a Cloud Run service using to_a2a.
"""
...
def deploy_planner_agent():
    """Deploy the Planner Agent to Cloud Run."""
    logger.info("=" * 60)
    logger.info("DEPLOYING PLANNER AGENT TO CLOUD RUN")
    logger.info("=" * 60)
    ...
    cmd = [
        ...
        ".",
        "--config", "agents/planner/cloudbuild.yaml",
        "--substitutions",
    ...
    logger.info("=" * 60)
    logger.info(f"\nPlanner Agent is now available at: {service_url}")
