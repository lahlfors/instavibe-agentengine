#!/usr/bin/env python3
"""
Deploy Platform MCP Client Agent to Cloud Run with A2A support.
This script deploys the Platform MCP Client Agent as a Cloud Run service using to_a2a.
"""
...
# Note: MCP Server address should be retrieved dynamically or set in env
MCP_SERVER_ADDRESS = os.getenv("MCP_SERVER_ADDRESS", "https://mcp-tool-server-735503743752.us-central1.run.app")
...
def deploy_platform_agent():
    """Deploy the Platform MCP Client Agent to Cloud Run."""
    logger.info("=" * 60)
    logger.info("DEPLOYING PLATFORM MCP CLIENT AGENT TO CLOUD RUN")
    logger.info("=" * 60)
    logger.info(f"Project: {PROJECT_ID}")
    logger.info(f"Region: {REGION}")
    logger.info(f"Service: {SERVICE_NAME}")
    logger.info(f"Image: {IMAGE_PATH}")
    logger.info(f"MCP Server: {MCP_SERVER_ADDRESS}")
    ...
    cmd = [
        ...
        ".",
        "--config", "agents/platform_mcp_client/cloudbuild.yaml",
        "--substitutions",
        f"_SERVICE_NAME={SERVICE_NAME},"
        f"_REGION={REGION},"
        f"_IMAGE_PATH={IMAGE_PATH},"
        f"_GEMINI_MODEL={GEMINI_MODEL},"
        f"_MCP_SERVER_ADDRESS={MCP_SERVER_ADDRESS}",
        "--project", PROJECT_ID
    ]
    ...
    logger.info("=" * 60)
    logger.info(f"\nPlatform MCP Client Agent is now available at: {service_url}")
