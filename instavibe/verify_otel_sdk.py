#!/usr/bin/env python
import sys
import os
import site
# from opentelemetry.sdk.trace import TracerProvider # Import moved into try for better error isolation

print("--- Attempting to import OpenTelemetry SDK components ---")
sys.stdout.flush()
try:
    import opentelemetry.trace
    from opentelemetry.sdk.trace import TracerProvider

    print("Successfully imported OpenTelemetry SDK trace components (opentelemetry.trace, TracerProvider)")

    # Check instantiation of TracerProvider
    provider = TracerProvider()
    print("TracerProvider instantiated successfully.")

    # Optionally, try to get a tracer
    tracer = opentelemetry.trace.get_tracer(__name__)
    print("Successfully got a tracer instance: {}".format(tracer))

    sys.stdout.flush()
    sys.exit(0) # Explicitly exit with 0 on success

except ImportError as e_import:
    print("!!! FAILED to import OpenTelemetry SDK components (ImportError): {}".format(e_import))
    sys.stderr.flush()
except Exception as e_general:
    print("!!! FAILED to import OpenTelemetry SDK components (General Exception): {}".format(e_general))
    sys.stderr.flush()

# This part is reached only if an exception occurred in the try block (and sys.exit(0) was not called)
print("--- Diagnostics on failure ---")
print("Python sys.path on failure (re-printing):")
for p_fail in sys.path: print(p_fail)
sys.stdout.flush()

print("Site packages:")
site_pkgs = []
try:
    site_pkgs = site.getsitepackages()
    print(site_pkgs)
    if site_pkgs and len(site_pkgs) > 0 and os.path.exists(site_pkgs[0]):
        print("Contents of first site-package dir ({}):".format(site_pkgs[0]))
        try:
            print(os.listdir(site_pkgs[0]))
        except Exception as e_listdir:
            print("Error listing site-packages contents: {}".format(e_listdir))
    else:
        print("No site-packages found by site.getsitepackages() or first one is invalid/inaccessible")
except Exception as e_site:
    print("Error getting site packages: {}".format(e_site))

sys.stdout.flush()
sys.stderr.flush()
sys.exit(1)
